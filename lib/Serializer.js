...
sock.downloadMediaMessage = async (message) => {
    ...
    m.from = m.key.remoteJid ? decodeJid(m.key.remoteJid) : null;

    // Fallback: if m.from not set but participant looks like group
    if (!m.from && m.key.participant) {
        m.from = decodeJid(m.key.participant);
    }

    // Determine if group based on 'from' or remoteJid
    m.isGroup = (m.from && m.from.endsWith("@g.us")) ||
                (m.key.remoteJid && m.key.remoteJid.endsWith("@g.us")) || false;

    m.sender = null;
    if (m.isGroup) {
        m.sender = m.key.participant ? decodeJid(m.key.participant) : null;
    } else if (m.isSelf) {
